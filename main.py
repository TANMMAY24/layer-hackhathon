...
def farmerdetails():
    query=db.engine.execute(f"SELECT * FROM `farmer`") 
...
def search():
    if request.method=="POST":
        ...
        bio=farmer.query.filter_by(rollno=rollno).first()
...
@login_required
def delete(id):
    db.engine.execute(f"DELETE FROM `farmer` WHERE `farmer`.`id`={id}")
...
def edit(id):
    ...
    if request.method=="POST":
        ...
        # query=db.engine.execute(f"UPDATE `farmer` SET `rollno`='{rollno}',`sname`='{sname}',`sem`='{sem}',`gender`='{gender}',`branch`='{branch}',`email`='{email}',`number`='{num}',`address`='{address}'")
        post=farmer.query.filter_by(id=id).first()
        post.rollno=rollno
        post.sname=sname
        post.sem=sem
        post.gender=gender
    ...
    posts=farmer.query.filter_by(id=id).first()
...
def addfarmer():
    ...
    if request.method=="POST":
        ...
        num=request.form.get('num')
        address=request.form.get('address')
        query=db.engine.execute(f"INSERT INTO `farmer` (`rollno`,`sname`,`sem`,`gender`,`branch`,`email`,`number`,`address`) VALUES ('{rollno}','{sname}','{sem}','{gender}','{branch}','{email}','{num}','{address}')")
    

        flash("Booking Confirmed","info")


    return render_template('farmer.html',dept=dept)
@app.route('/test')
def test():
    try:
        Test.query.all()
